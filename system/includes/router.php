...
if (isset($_SERVER['REQUEST_URI'])) {
    ...
    if (count($page['call_parts']) > 1) {
        ...
        if (!empty($page['call_parts'][0])) {
            switch ($page['call_parts'][0]) {
                ...
                if ($page['basename'] == 'admin') {
                    if (file_exists(SYS_VIEWS_DIR . DS . 'welcome.php')) {
                        require_once SYS_VIEWS_DIR . DS . 'welcome.php';
                    }
                } else {
                    if (file_exists(SYS_VIEWS_DIR . DS . $page['fullpath'])) {
                        require_once SYS_VIEWS_DIR . DS . $page['fullpath'];
                    } else {
        ...
        // get the file
        $page['fullpath']	= $page['folder_names'] . $page['file_ext'];
